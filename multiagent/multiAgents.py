...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        for ghostState in newGhostStates:
            # estimate of utility, for any ghost
            if util.manhattanDistance(ghostState.getPosition(), newPos) < 2:
                return float("-inf")
        #calculate the food distance and sort it
        foodsPos = sorted(newFood.asList(), key=lambda pos: util.manhattanDistance(newPos, pos))
        foodMinDist = 0 if len(foodsPos)==0 else util.manhattanDistance(foodsPos[0], newPos)
        #food penalty
        foodsPen = -foodMinDist - 10*successorGameState.getNumFood()
    
        return successorGameState.getScore() + foodsPen
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        # walk for all legal actions, to find correct action
    ...
    def maxValue(self, gameState, currentDepth):
        # find the action make value max
        values = [self.getValue(gameState.generateSuccessor(0, action), currentDepth, 1) for action in gameState.getLegalActions(0)]
        return max(values)
...
def betterEvaluationFunction(currentGameState):
    ...
    # current socre subtract food penalty, bonus for closer distance to ghost, 
    # very likely to ReflexAgent.evaluationFunction,
    # except consideration for ghost.
